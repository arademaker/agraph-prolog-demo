...
inductive Grandparent : Person → Person → Prop where
  | intro (mid : Person) : Parent x mid → Parent mid z → Grandparent x z

-- In Prolog: sibling(X, Y) :- parent(Z, X), parent(Z, Y), X \= Y.

inductive Sibling : Person → Person → Prop where
  | intro (par : Person) : Parent par x → Parent par y → x ≠ y → Sibling x y
...
class IsParent (x : Person) (y : outParam Person) where

instance : IsParent tom bob where
instance : IsParent tom liz where
instance : IsParent bob ann where
instance : IsParent bob pat where

-- Rule: grandparent(X, Z) :- parent(X, Y), parent(Y, Z).
-- The resolver: given x, finds y via IsParent x y,
--               then finds z via IsParent y z.
class IsGrandparent (x : Person) (z : outParam Person) where

instance [IsParent x y] [IsParent y z] : IsGrandparent x z where
...
-- It never tries bob → pat. This shows the same limitation as
-- sibling: outParam makes the relation functional (a single result).
-- outParam makes the relation functional — only finds the first grandchild:
/--
...
class IsSibling (x y : Person) where

instance : IsSibling bob liz where
instance : IsSibling liz bob where
instance : IsSibling ann pat where
instance : IsSibling pat ann where
